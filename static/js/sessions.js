...
function _deselectCurrentSession(sid) {
  ...
  uiModule.el('current-meta').textContent = 'Sovi Chat';
...
export async function selectSession(id, { keepSidebar = false } = {}) {
  ...
  try {
    ...
    // URL hash — the user complained that coming back to Sovi kept
    ...
    if (currentMetaEl) {
      currentMetaEl.textContent = meta ? meta.name : 'Sovi Chat';
    }
